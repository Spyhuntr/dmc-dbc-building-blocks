...
dash.register_page(__name__, path='/stats', title='Home')
...
layout = dmc.LoadingOverlay([dmc.Grid(
    children=[
        ...
        dmc.Col(
            dmc.Title(f"Stats", order=1)
        ),
        dmc.Col(
            id="stats-sample-container",
            children=[]
        )
    ])])
...
@callback(
    Output('stats-sample-container', 'children'),
    [Input('url', 'pathname'),
     Input('stats-sample-container', 'children')]
)
...
@callback(
    Output({'type': 'stats-rendering', 'index': MATCH}, 'children'),
    [Input('url', 'pathname'),
     Input({'type': 'stats-code-switch', 'index': MATCH}, 'checked')],
    State({'type': 'stats-code-switch', 'index': MATCH}, 'id')
)
def state_change(_, switch, id):
    ...
    for name in card_dict[id['index']]['file'].namelist():
        ...
        if name.endswith('css'):
            css_file = card_dict[id['index']]['file'].read(name).decode('utf-8')

    img_file = 'https://dmc-dbc-building-blocks.s3.amazonaws.com/examples/stats/images/' + \
